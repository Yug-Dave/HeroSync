<template>
  <div class="wrap">
    ...
    <ReportSummary v-if="report" title="Monthly Summary" :report="report" />
...
async function load() {
  ...
  report.value = null;
  try {
    report.value = await getMonthlyReport(year.value, month.value);
  } catch (e) {
    console.error(e);
    error.value = "Could not load monthly report. Check backend/CORS/session.";
  } finally {
    loading.value = false;
  }
}
...
.field { display: grid; gap: 8px; min-width: 140px; }
