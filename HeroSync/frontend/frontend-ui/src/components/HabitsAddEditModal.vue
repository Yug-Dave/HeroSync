...
watch(() => props.isOpen, (open) => {
  ...
  form.value = props.editMode && props.initialData
    ? { 
        ...
        xpValue: props.initialData.xpValue ?? 60
      }
        ...
        <div class="modal-panel" role="dialog" aria-modal="true">
          <div class="modal-header">
            <div class="modal-title-group">
              <span class="modal-icon">
                <svg v-if="editMode" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="color:var(--gold)"><path d="M17 3a2.828 2.828 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5L17 3z"/></svg>
                <svg v-else width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="color:var(--accent)"><path d="M14.5 17.5L3 6V3h3l11.5 11.5"/><path d="M13 19l6-6"/><path d="M16 16l4 4"/><path d="M19 21l2-2"/></svg>
              </span>
              <h3 class="modal-title">{{ editMode ? 'Edit Quest' : 'New Quest' }}</h3>
            </div>
            <button class="close-btn" @click="$emit('close')" :disabled="saving">
          ...
          <div class="modal-body">
            <div class="form-group">
              ...
              <input v-model="form.name" class="form-input" placeholder="e.g. Morning Run" autofocus :disabled="saving" />
            </div>
            <div class="form-group">
              <label class="form-label">Description <span class="optional">(Optional)</span></label>
              <input v-model="form.description" class="form-input" placeholder="e.g. 30 mins around the park" :disabled="saving" />
            </div>
            <!-- Tier Selector -->
            <div class="form-group">
          ...
          <div class="modal-footer">
            ...
            <button class="btn-save" @click="handleSubmit" :disabled="saving || !form.name.trim()">
              {{ saving ? 'Saving…' : editMode ? 'Save Changes' : 'Create Quest' }}
            </button>
          </div>
        </div>
      </div>
...
.form-label { font-size: .78rem; font-weight: 700; color: var(--muted); text-transform: uppercase; letter-spacing: .4px; }
...
.error-text { color: #ef4444; font-size: .85rem; margin: 0; }
